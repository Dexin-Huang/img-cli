...
async function extractMark(args) {
  ...
  console.error('[1/2] Asking gpt-image-2 to create a clean mark from the board...');
  const result = await provider.call({
    prompt: 'create a clean mark from this brand board. Render it large and centered as a pure solid black silhouette on a pure solid white background, with generous padding, vector-clean edges, no other elements.',
    images: [inputBoard],
    size: '1K', ratio: '1:1', quality: 'high',
    apiKey: provider.apiKey, model: provider.model,
  });
  if (!result) { console.error('Mark creation failed'); process.exit(1); }
  ...
  if (!fs.existsSync(venvPython())) {
    if (process.stdin.isTTY) {
      ...
      else { console.error("PNG saved. Run 'img install --venv' to enable SVG output."); console.log(outputPng); return; }
    } else {
      console.error("PNG saved. Run 'img install --venv' to enable SVG output.");
      console.log(outputPng);
      return;
    }
  }

  console.error('[2/2] Vectorizing with potrace...');
  ...
  if (py.status !== 0) { console.error('Vectorization failed:', (py.stderr || '').slice(0, 500)); process.exit(1); }
